...
-- ================= TURMAS =================
-- Uma turma é uma leva de alunos que entrou junto e vai se formar (sair da
-- escola) junto — apelido próprio (ex: "Fera Azul"), não um catálogo fixo:
-- podem existir várias turmas do mesmo curso/série ao mesmo tempo (ex: duas
-- CFS na mesma série, como ocorreu na época do Covid), por isso "nome" é o
-- identificador, não curso+série. "Formar turma" (issue #32) desativa a
-- turma E todos os alunos vinculados a ela de uma vez, quando ela sai da
-- escola — ver core/turmas_core.php::formarTurma().
CREATE TABLE turmas (
  id INT AUTO_INCREMENT PRIMARY KEY,
...
-- ================= PERMISSÃO: TURMAS =================
-- Criar/"formar" (desativar turma + todos os alunos dela) é decisão de nível
-- CA, não de esquadrão — mesmos cargos de gerenciar_dominio, não os de
-- editar_efetivo. Ver issue #32 e core/turmas_core.php.
INSERT INTO permissoes (chave, descricao) VALUES
